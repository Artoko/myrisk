...
Public Class frmUVCE
    Private m_UVCE As New FireBlast.UVCE '用TNT当量法进行计算的对象
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Property UVCE() As FireBlast.UVCE
        Get
            Return Me.m_UVCE
        End Get
        Set(ByVal value As FireBlast.UVCE)
            Me.m_UVCE = value
        End Set
    End Property
    ...
    Private Sub cmdSeachData_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSeachData.Click
        ...
        If frmNew.DialogResult = Windows.Forms.DialogResult.OK Then
            If frmNew.txtHc.Text <> "" Then
                Me.m_UVCE.Qf = frmNew.txtHc.Text
    ...
    Private Sub UpdateDestroyR()
        Dim L As Integer = Me.m_UVCE.DestroyRr.Length
        Me.EFlex1.Rows.Count = L + 1
        For i As Integer = 0 To L - 1
            Me.EFlex1.SetData(1 + i, 0, Me.m_UVCE.DestroyRr(i).MDrangName)
            Me.EFlex1.SetData(1 + i, 1, Me.m_UVCE.DestroyRr(i).MDrang)
            Me.EFlex1.SetData(1 + i, 2, Me.m_UVCE.DestroyRr(i).Selected)
        Next
    End Sub
    ...
    Private Sub frmUVCE_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '初始化 测试值
        txtLeakSourceName.Text = Me.m_UVCE.SourceName
        Me.txtName.Text = Me.m_UVCE.Name
        Me.txtX.Value = Me.m_UVCE.SoucePoint.x
        Me.txtY.Value = Me.m_UVCE.SoucePoint.y

        txta.Value = Me.m_UVCE.a
        Me.txtQf.Value = Me.m_UVCE.Qf
        txtWf.Value = Me.m_UVCE.Wf

        CTextStep.Value = Me.m_UVCE.Tstep
        CTextCount.Value = Me.m_UVCE.Tcount
        ...
        Me.chkReflect.Checked = Me.m_UVCE.Reflect '考点地面反射
        CheckFormula.Checked = Me.m_UVCE.FormulaChecked '用公式法计算死亡半径和财产损失半径
    ...
    Private Sub cmdDamageAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdDamageAdd.Click
        ReDim Preserve Me.m_UVCE.DestroyRr(Me.m_UVCE.DestroyRr.Length) '长度增加1
        Me.m_UVCE.DestroyRr(Me.m_UVCE.DestroyRr.Length - 1) = New FireBlast.Drang  '创建一个对象
        UpdateDestroyR()
    End Sub
    ...
    Private Sub cmdDamageDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdDamageDelete.Click
        ...
        If index >= 0 Then
            For i As Integer = index To Me.m_UVCE.DestroyRr.Length - 2
                Me.m_UVCE.DestroyRr(i) = Me.m_UVCE.DestroyRr(i + 1)
            Next
            ReDim Preserve Me.m_UVCE.DestroyRr(0 To Me.m_UVCE.DestroyRr.Length - 2)
    ...
    Public Sub SaveUVCE() '保存
        ...
        'End If
    End Sub
    Private Sub OutPutUVCE(ByRef sFileName As String) '导出UVCE（TNT模型）预测数据
        'Dim i As Short
        'i = 0
        ...
        'FileClose(1)
    End Sub
    Public Sub InPutUVCE(ByRef sFileName As String) '导入UVCE（TNT模型）预测数据
    ...
    Private Sub chkReflect_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkReflect.CheckedChanged
        Me.m_UVCE.Reflect = Me.chkReflect.Checked
    End Sub
    ...
    Private Sub CTextStep_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles CTextStep.Validated
        Me.m_UVCE.Tstep = Me.CTextStep.Value
    ...
    Private Sub CTextCount_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles CTextCount.Validating
        Me.m_UVCE.Tcount = Me.CTextCount.Value
    ...
    Private Sub EFlex1_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles EFlex1.Validating
        ...
        ReDim Me.m_UVCE.DestroyRr(0 To L - 1)
        For i As Integer = 0 To L - 1
            m_UVCE.DestroyRr(i) = New FireBlast.Drang

            Me.m_UVCE.DestroyRr(i).MDrangName = Me.EFlex1.GetData(i + 1, 0)
            m_UVCE.DestroyRr(i).MDrang = Me.EFlex1.GetData(i + 1, 1)
            Me.m_UVCE.DestroyRr(i).Selected = Me.EFlex1.GetData(i + 1, 2)
        Next
    End Sub
    ...
    Private Sub CheckFormula_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CheckFormula.CheckedChanged
        Me.m_UVCE.FormulaChecked = CheckFormula.Checked
    End Sub
    ...
    Private Sub txtLeakSourceName_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtLeakSourceName.Validating
        m_UVCE.SourceName = txtLeakSourceName.Text
    End Sub
    ...
    Private Sub txtX_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtX.Validating
        Me.m_UVCE.SoucePoint.x = txtX.Value
    End Sub
    ...
    Private Sub txtY_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtY.Validating
        Me.m_UVCE.SoucePoint.y = txtY.Value
    End Sub
