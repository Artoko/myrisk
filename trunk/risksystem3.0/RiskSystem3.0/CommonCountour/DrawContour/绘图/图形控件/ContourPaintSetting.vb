...
<Serializable()> Public Class ContourPaintSetting
    ...
    Public Sub ResetCountData()
        If Me.m_ChartType = 0 Then
            ...
            If ContourPannel.Contours.ArrayContour.Length > 0 Then
                ReDim Preserve ContourPannel.Contours.ArrayContour(0 To Me.ContourPannel.Contours.ContourValueSetting.ContourValue.Length - 1)
            Else
                ReDim ContourPannel.Contours.ArrayContour(0 To Me.ContourPannel.Contours.ContourValueSetting.ContourValue.Length - 1)
            End If
            For i As Integer = 0 To Me.ContourPannel.Contours.ContourValueSetting.ContourValue.Length - 1
                ContourPannel.Contours.ArrayContour(i) = New Contour
                ContourPannel.Contours.ArrayContour(i).ContourName = Me.ContourPannel.Contours.ContourValueSetting.ContourNames(i)
                ContourPannel.Contours.ArrayContour(i).ContourValue = Me.ContourPannel.Contours.ContourValueSetting.ContourValue(i)

            Next
            For i As Integer = 0 To Me.ArrayHurtValue.Length - 1
                ContourPannel.Contours.ArrayContour(i).HurtValue = ArrayHurtValue(i)
            Next
            If Me.ContourPannel.Contours.ContourValueSetting.ContourValue.Length > 0 Then
    ...
    ''' <remarks></remarks>
    Public Sub Clear()
        ReDim Me.ContourPannel.Contours.ContourValueSetting.ContourNames(-1)  '等值线的名称
        ReDim Me.ContourPannel.Contours.ContourValueSetting.ContourValue(-1)  '等值线的值
    ...
    Public Sub SetGrid(ByVal Xinit As Single, ByVal Xdelta As Single, ByVal Xnum As Single, ByVal yinit As Single, ByVal ydelta As Single, ByVal ynum As Single)
        ...
        ContourPannel.Axes.BottomAxis.MainAxisScale.Min = Xinit
        ContourPannel.Axes.BottomAxis.MainAxisScale.Max = Xinit + (Xnum - 1) * Xdelta
        ContourPannel.Axes.BottomAxis.MainAxisScale.Increase = Xdelta
        ContourPannel.Axes.LeftAxis.MainAxisScale.Min = yinit
        ContourPannel.Axes.LeftAxis.MainAxisScale.Max = yinit + (ynum - 1) * ydelta
        ContourPannel.Axes.LeftAxis.MainAxisScale.Increase = ydelta
        ReDim GridPoint(m_nRows - 1, m_nCols - 1) '网格点数组，从(0，0)开始，用于绘制等值线图
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function Clone() As ContourPaintSetting
        Try
            ' Create a filestream object
            Dim fileStr As Stream = File.Open(ApplicationPath() & "ContourPaintSetting.sav", FileMode.Create)
            ' Create a linked list object and populate it with random nodes
            Dim ObjContourPaintSetting As New ContourPaintSetting
            ObjContourPaintSetting = Me
            ...
            formatter.Serialize(fileStr, ObjContourPaintSetting)
            ' All done
            fileStr.Close()
        ...
        Try
            Dim fileStr As Stream = File.Open(ApplicationPath() & "ContourPaintSetting.sav", FileMode.Open)
            ...
            Dim ObjContourPaintSetting As New ContourPaintSetting
            ObjContourPaintSetting = CType(formatter.Deserialize(fileStr), ContourPaintSetting)
            fileStr.Close()
            Return ObjContourPaintSetting
        Catch ex As Exception
            MsgBox("读等值线文件错误!")
            Return Nothing
        End Try
    ...
    <OnSerializing()> _
     Friend Sub OnSerializingMethod(ByVal context As StreamingContext)
        '用于将版本1.3.0.3升级至1.3.0.4
        Me.ContourValueSetting = Nothing
    End Sub
